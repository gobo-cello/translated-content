...
- `*`（ワイルドカード）
  - : `Accept-Language` フィールドに列挙されていない任意の言語に一致します。
- `;q=` (Q 値の重み)
