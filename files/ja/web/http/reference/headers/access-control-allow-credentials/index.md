...
HTTP の **`Access-Control-Allow-Credentials`** は{{Glossary("response header", "レスポンスヘッダー")}}で、サーバーがオリジンをまたいだ HTTP リクエストに資格情報を添付することを許可するかどうかをブラウザーに伝えます。
...
クライアントは、サイト間リクエストで資格情報を添付することを、いくつかの方法で要求できます。
...
- {{glossary("Preflight_request", "プリフライト")}}のあるリクエストの場合、プリフライトとリクエストには資格情報を添付しません。
